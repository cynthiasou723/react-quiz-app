          ...
          (
            <div className="topics-container">
              ...
              {this.state.topics[this.state.currentIndex].answers.map((answer, index) => {
                return (
                  <button key={index} className={ `${this.state.buttonClass[index]} ${this.state.isDisabled ? 'disabled' : ''}`} onClick={() => this.next(index, answer.correct)}>{ answer.value }</button>
                )
              })}
            </div>
          ) :
          (
            <div className={ this.state.corrects === this.state.topics.length ? 'fireworks' : ''}>
